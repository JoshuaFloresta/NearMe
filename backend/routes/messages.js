...
router.post('/conversations/:id/messages', requireAuth, async (req, res) => {
  try {
    ...
    // Validate message text is present and not just whitespace.
    const { text } = req.body;
    if (!text?.trim()) {
    ...
    const message = {
      ...
      text: text.trim(),
      // Support attachments array (frontend may send file metadata here).
      attachments: Array.isArray(req.body.attachments) ? req.body.attachments : [],
      // `readBy` initially contains the sender so their UI treats it as read.
      readBy: [sender.id],
      createdAt: now,
    };
